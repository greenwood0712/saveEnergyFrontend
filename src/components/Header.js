    ...
    <>
      {search && (
        <nav className="search-input w-full fixed border-0 bg-white flex justify-start items-center text-2xl text-black py-4 z-50">
          ...
          <button onClick={(e) => {setSearch(false); document.body.style.overflow = "auto";}} className='flex ml-10 px-5 border-l-2 border-gray-400'>
            <XIcon className="block h-8 w-8 text-green-700" aria-hidden="true" />
              ...
              <ul className="justify-center items-center space-y-8 lg:flex lg:space-x-6 lg:space-y-0">
                {navigations.map((item, idx) => {
                  return (
                    <li
                      key={idx}
                      className="p-3 m-1 hover:border-b-2 hover:border-green-500"
                    >
